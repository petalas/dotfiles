...
It fast-forwards this repository, resolves the saved installation plan, reconciles selected missing software/state, then upgrades software already present—including deselected applications. Deselection never suppresses maintenance or triggers uninstallation. Independent failures are collected and reported at the end. The complete output is saved with mode 0600 in `${XDG_STATE_HOME:-~/.local/state}/dotfiles/latest-update.log`, and failed summaries repeat that path.
