...
**Remove** immediately cancels a pending installation when the application is already absent. Otherwise it prefers the exact package-manager registration or matching installation receipt; if exact custody is unavailable, it automatically uses the application's catalog-reviewed cleanup recipe and discloses that fallback during review and confirmation. Retained dependents block prerequisite removal; package-manager dependency checks are never bypassed. Removal never deletes package-manager orphans, shared prerequisites, projects, profiles, vaults, sessions, editor configuration, browser data, Docker data, or other user data. Changed support files are retained rather than deleted.
