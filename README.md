...
`upd` checks installed GitHub skills by source and ref, compares their folder hashes, and installs changed skills in one batch per source/ref. It uses `gh api` when available and falls back to Git for the tree lookup. Other source types and moved skill paths use the CLI's update behavior. Failed checks and installations fail the skills step while independent sources continue. Run `./tools/update-ai-skills --check` to preview the batches without installing or pruning links. `./install ai_skills` remains the full catalog install/repair command, including skills not currently installed.
...
- On macOS, setup runs the selected font step immediately after plan confirmation, before application installation. It imports the generated `Seashells` and `Seashells Light` Terminal profiles with Hack Nerd Font Mono and the palettes under `dot/.config/ghostty/themes/`. Terminal cannot switch profiles with system appearance, so setup keeps `Seashells` as the default and startup profile and applies it to open tabs. Regenerate the tracked profiles with `/usr/bin/osascript -l JavaScript tools/generate-macos-terminal-profile.js dot/.config/ghostty/config.ghostty dot/.config/ghostty/themes/seashells Seashells >dot/terminal/SeaShells.terminal` and `/usr/bin/osascript -l JavaScript tools/generate-macos-terminal-profile.js dot/.config/ghostty/config.ghostty dot/.config/ghostty/themes/seashells-light 'Seashells Light' >dot/terminal/Seashells-Light.terminal`.
...
- `./tools/lint-shell` runs ShellCheck plus Bash/Zsh syntax validation over every tracked shell script, exactly as the `checks` job does; `.githooks/pre-commit` runs it over staged files and the linker activates the hook through `core.hooksPath`. `./tests/live/test-macos-inspection.sh` runs the read-only macOS inspection smoke locally through the verified release binary.
