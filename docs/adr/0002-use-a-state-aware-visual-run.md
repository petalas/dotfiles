...
Count run progress at the logical-operation boundary rather than at the application boundary. Every selected package-manager entry, declared phase within a direct installer (for example nvm and Node, or rustup and the stable toolchain), removal method, standalone setup step, and post-operation application verification is a separate logical operation. Efficient package-manager batches remain shared transactions, but expose and settle every catalog entry they contain so batching does not hide planned work.
