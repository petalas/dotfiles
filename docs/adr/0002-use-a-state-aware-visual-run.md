...
Persist only version-1 wanted/not-wanted plan records. The prepared-run review is the confirmation screen: Enter writes a SHA-256 approval for the exact version-2 artifact displayed, while Esc cancels; no typed application name or removal phrase is required. Do not persist destructive outcomes. Execution derives its final removal result from reinspection and writes a permission-restricted, non-replayable report.
