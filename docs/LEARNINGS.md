...
- `link-dotfiles.sh` installs all three tracked configs together: `config.yml` for default, `config-work.yml` for work, and `config-cheap.yml` for cheap. Selection is launch-time `--profile` or `OMP_PROFILE`, not the default config symlink. Explicit `--profile` wins over the environment; an unselected launch uses default. Relinking an old work-selected symlink restores default there and backs up the replaced link as `config.yml.old`; work now has its own profile path. Existing authentication and session history stay under default.
- OMP's native partial-save preserves each symlink and updates its target. Real `omp --profile <name> config set modelRoles ...` smoke checks verify independent write-through for all three profiles and preservation across relinking. Keep each tracked YAML as its settings definition; a linker merge block would overwrite later UI choices.
