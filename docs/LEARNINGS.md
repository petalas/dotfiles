...
- JXA can expose a nested `NSUserDefaults` dictionary as either an Objective-C bridge or a plain JavaScript object. Wrap every external dictionary with `$()` before calling Foundation methods. Native tests must exercise both shapes because plist fixtures can expose only the bridged shape on the test host.
