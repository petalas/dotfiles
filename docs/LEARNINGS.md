...
- `link-dotfiles.sh` defaults `~/.omp/agent/config.yml` to the personal `dot/.omp/agent/config.yml`, but preserves an existing symlink to this checkout's `dot/.omp/agent/config-work.yml`, including when called by `upd`. The symlink is the machine-local selection; no separate flag or state file is needed. Match target identity with `-ef` so absolute and relative work symlinks both survive. OMP's native partial-save preserves the symlink and updates its target, verified with real `omp config set` calls for model roles, fallback chains, and ordinary settings. Keep each tracked YAML as its settings definition; a linker merge block would overwrite later UI choices.
- Check the effective value with `omp config get SETTING --json`. Reloading plugins does not move settings between these files.
- The model selector saves global assignments under `modelRoles`, while retry fallback selectors live separately under `retry.fallbackChains`. Both now appear directly in the dotfiles diff. `modelRoleStorage: project` and named profiles write elsewhere. `upd` refuses a dirty checkout, so commit or stash OMP changes before syncing.
- Link only the config file and themes, never the whole agent directory. Authentication and session state remain local; secrets must not be added to the tracked config. On first linking, preserve the existing config as `config.yml.old` rather than importing machine-local data into Git. OMP's adjacent `config.yml.lock` and `config-work.yml.lock` are ignored.
- Symlink-write tests must use a disposable checkout so they cannot modify real tracked settings. Real OMP config commands also open `agent.db`; give smoke checks an isolated `PI_CODING_AGENT_DIR` and let OMP create its database instead of using a text sentinel as a fake SQLite file. Compare symlink target identity in these fixtures, or resolve both paths: macOS temporary paths may differ only by the `/private` prefix.
