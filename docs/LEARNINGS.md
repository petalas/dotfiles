...
- `link-dotfiles.sh` symlinks `~/.omp/agent/config.yml` to `dot/.omp/agent/config.yml`. OMP's native partial-save preserves the symlink and updates its target, verified with real `omp config set` calls for model roles, fallback chains, and ordinary settings. Keep the tracked YAML as the only settings definition; a linker merge block would overwrite later UI choices.
...
- Symlink-write tests must use a disposable checkout so they cannot modify real tracked settings. Real OMP config commands also open `agent.db`; give smoke checks an isolated `PI_CODING_AGENT_DIR` and let OMP create its database instead of using a text sentinel as a fake SQLite file.
