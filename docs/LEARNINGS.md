...
- Cheap pins all nine built-in roles, including `slow`, `vision`, and `commit`, to `opencode-go/muse-spark-1.3-contributor`. Leaving roles unset can select other models through OMP's role defaults. Do not copy the default/work GPT fallback chains into cheap; its `retry.modelFallback` is false.
