...
- Configure APT mirrors before that first bootstrap refresh: Debian uses its official `deb.debian.org` CDN, while Ubuntu's primary archive uses Canonical's requester-local `mirrors.ubuntu.com/mirrors.txt` service with APT-managed fallback. Keep `security.ubuntu.com` unchanged so security updates do not wait for downstream mirrors to synchronize.
