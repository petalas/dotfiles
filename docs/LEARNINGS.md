...
- JXA can expose a nested `NSUserDefaults` dictionary as an Objective-C bridge, a plain JavaScript object, or a proxy without `isKindOfClass`. `$()` can return that proxy unchanged. Use `ObjC.unwrap`, validate the resulting JavaScript record, and copy its entries into a new `NSMutableDictionary`. Native tests must exercise all three shapes because plist fixtures can expose only the ordinary bridge on the test host.
