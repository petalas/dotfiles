...
- Configure APT mirrors before that first bootstrap refresh: Debian uses its official `deb.debian.org` CDN. Ubuntu normally uses Canonical's requester-local `mirrors.ubuntu.com/mirrors.txt` service with APT-managed fallback, but cloud egress can receive a one-entry list pointing back to the slow primary archive. The integration runner therefore benchmarks the official country, Azure, and primary endpoints before Docker starts and passes the fastest verified endpoint into the bootstrap. Keep `security.ubuntu.com` unchanged so security updates do not wait for downstream mirrors to synchronize.
