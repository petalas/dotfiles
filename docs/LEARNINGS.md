...
- `link-dotfiles.sh` installs all four tracked configs together: `config.yml` for default, `config-work.yml` for work, `config-cheap.yml` for cheap, and `config-free.yml` for free. Selection is launch-time `--profile` or `OMP_PROFILE`, not the default config symlink. Explicit `--profile` wins over the environment; an unselected launch uses default. Relinking an old work-selected symlink restores default there and backs up the replaced link as `config.yml.old`; work now has its own profile path. Existing authentication and session history stay under default.
...
- Query settings with `omp --profile cheap config get modelRoles --json`. The CLI accepts schema keys, not arbitrary YAML containers: query `theme.dark` and `theme.light` separately, not `theme`. Reloading plugins does not switch profiles or move settings between files.
...
- Cheap pins `default`, `task`, `smol`, `plan`, `tiny`, `advisor`, and `slow` to `opencode-go/deepseek-v4.1-flash`, with `tiny` at `minimal` effort and `plan`/`slow` at `xhigh`. Leaving roles unset (`vision`, `commit`) can select other models through OMP's role defaults. Do not copy the default/work GPT fallback chains into cheap; its `retry.modelFallback` is false.
- A `:effort` suffix outside a model's advertised ladder is silently clamped **down** to the highest supported level, never rejected. `opencode-go/deepseek-v4.1-flash` advertises only `low`, `high`, `max`, so cheap's `tiny: …:minimal` resolves to `low`, and its `advisor: …:medium` and `plan`/`slow: …:xhigh` all resolve to `high` (2026-09-17). Pick suffixes from `omp models find <id> --json`'s `thinking` array — the clamp ladder lives in `clampThinkingLevelForModel` (`packages/catalog/src/model-thinking.ts`).
