...
- Terminal stores profile fonts and colors as native keyed archives inside the `com.apple.Terminal` preference domain. Keep the generated `dot/terminal/SeaShells.terminal` and `dot/terminal/Seashells-Light.terminal` exports as the native artifacts, and derive them from the corresponding Ghostty themes with `tools/generate-macos-terminal-profile.js`.
- The canonical dark and light SeaShells palettes are pinned to [`odysseyalive/omarchy-seashells-theme@00dca31`](https://github.com/odysseyalive/omarchy-seashells-theme/tree/00dca31761374d5526790dd8a10271edbc6f9ec8/omarchy/themes). Palette changes must update Ghostty, Kitty, Pi, OMP, the Powerlevel10k literal-color allowlists, and both generated Terminal profiles together; `tests/test-seashells-palette.sh` guards that contract. Shell and Powerlevel10k numeric mappings use ANSI indexes and do not need remapping when slot colors change.
- The `defaults` manual warns that a running application can ignore or overwrite external preference changes. A polling launchd job also misses short quit/relaunch gaps, leaving the profile unapplied indefinitely. When Terminal is running, open the native `.terminal` exports through Terminal itself, set the dark profile as its default and startup setting through Terminal's scripting interface, and assign it to every existing tab. Terminal always opens an import window and its scripting API cannot dismiss the shell-confirmation sheet non-interactively, so terminate that throwaway shell and hide its window. This changes the installer tab immediately without touching the live preference domain externally. Direct preference reconciliation remains safe when Terminal is not running.
- Reconcile only `Window Settings.Seashells`, `Window Settings.Seashells Light`, `Default Window Settings`, and `Startup Window Settings`. Preserve every other Terminal profile and top-level preference. Keep a fixture-plist mode in the apply helper so tests never write the host's Terminal preferences.
- JXA can expose a nested `NSUserDefaults` dictionary as an Objective-C bridge, a plain JavaScript object, a proxy without `isKindOfClass`, or an object with a nonstandard type tag. Do not infer whether it is a dictionary from JXA methods or JavaScript type tags. Pass the value to `NSMutableDictionary.dictionaryWithDictionary`; Foundation accepts dictionary representations and rejects arrays, functions, and scalar values. Native tests must exercise each shape because plist fixtures expose only the ordinary bridge on the test host.
...
- Updater tests must set `SDKMAN_DIR` and `XDG_CONFIG_HOME` to their fixture. Replacing `HOME` alone does not override an inherited SDKMAN path and can make a supposedly isolated test update the host's real candidates. GitHub's Ubuntu runners export `XDG_CONFIG_HOME=/home/runner/.config`, so the updater then looked for the Neovim fixture outside the test home and the `nvim plugins` assertion failed only on Linux, silently. `tests/run.sh` now names the file, line, and command of any bare assertion that fails; reproduce Linux-only environment leaks on macOS with `XDG_CONFIG_HOME=/tmp/elsewhere ./tests/run.sh`.
...
- OMP reads the default profile from `~/.omp/agent/config.yml` and named profiles from `~/.omp/profiles/<name>/agent/config.yml`. `~/.pi/agent/settings.json` is a legacy migration source and changes there are ignored after the YAML configuration exists.
- `link-dotfiles.sh` installs all four tracked configs together: `config.yml` for default, `config-work.yml` for work, `config-deepseek.yml` for deepseek, and `config-free.yml` for free. Selection is launch-time `--profile` or `OMP_PROFILE`, not the default config symlink. Explicit `--profile` wins over the environment; an unselected launch uses default. Relinking an old work-selected symlink restores default there and backs up the replaced link as `config.yml.old`; work now has its own profile path. Existing authentication and session history stay under default.
- OMP's native partial-save preserves each symlink and updates its target. Real `omp --profile <name> config set modelRoles ...` smoke checks verify independent write-through for all four profiles and preservation across relinking. Keep each tracked YAML as its settings definition; a linker merge block would overwrite later UI choices.
- Query settings with `omp --profile deepseek config get modelRoles --json`. The CLI accepts schema keys, not arbitrary YAML containers: query `theme.dark` and `theme.light` separately, not `theme`. Reloading plugins does not switch profiles or move settings between files.
- Global model assignments and retry fallbacks live under the selected profile's `modelRoles` and `retry.fallbackChains`. `modelRoleStorage: project` writes to the project's `.omp/config.yml` instead. Project configuration and explicit model overrides take precedence over profile settings. `upd` refuses a dirty checkout, so commit or stash OMP changes before syncing.
- DeepSeek pins every profile role, `vision` and `commit` included, to `opencode-go/deepseek-v4.1-flash`, so no role can select another model. `default`, `advisor`, and `vision` sit at `high`, `plan` and `slow` at `max`, `smol`, `tiny`, and `commit` at `low`, and `task` keeps `auto` so the bundled task agent classifies effort per prompt. Do not copy the default/work GPT fallback chains into deepseek; its `retry.modelFallback` is false.
- A `:effort` suffix outside a model's advertised ladder is silently clamped **down** to the highest supported level, never rejected. `opencode-go/deepseek-v4.1-flash` advertises only `low`, `high`, `max`, so a `:minimal` suffix there resolves to `low` and `:medium` or `:xhigh` both resolve to `high` (2026-09-17). Pick suffixes from `omp models find <id> --json`'s `thinking` array, and check every existing suffix when a role moves to a model with a shorter ladder. `auto` is a selector rather than a ladder rung, so the clamp never applies to it: OMP builds its CLI level list as `["off", ...ladder, "auto"]`, and this profile's `task: …:auto` was never among the flagged suffixes. The clamp ladder lives in `clampThinkingLevelForModel` (`packages/catalog/src/model-thinking.ts`).
- The default profile splits planning from implementation: `default`, `plan`, `slow`, and `advisor` stay on `openai-codex/gpt-6-astra`, while `task`, `smol`, and `tiny` are `opencode-go/deepseek-v4.1-flash`. `prewalk.enabled: true` hands the session from astra to the `smol` role at the first `edit`/`write` **after** the `todo` gate opens, so `smol` is the bulk implementer.
- `smol` deliberately carries **no** `:effort` suffix, because its two consumers resolve the level by different rules. Subagents take their definition's `thinking-level` when the pattern has no explicit suffix, so `scout`/`sonic` land on `low` (their declared `medium` clamped down) and stay fast. The prewalk handoff passes no level at all, so `setModelTemporary` keeps the session's current level clamped onto the target — astra `xhigh` becomes deepseek `high` for the implementer. Adding a suffix to `smol` collapses both paths onto one level: `:high` slows every scout/sonic spawn, `:low` drops the bulk implementer to deepseek's weakest tier. An explicit suffix also outranks an agent definition's `thinking-level` (`packages/coding-agent/src/task/executor.ts`, "Precedence: caller `effort` > explicit `:level` suffix > agent-definition default"). Verified 2026-09-17: scout child session `low`, prewalk handoff `high`.
- Prewalk only fires when the strong model writes its todo list before its first mutating call. A one-file task whose first action is the `write` opens the gate too late, so the session stays on the planning model for the whole run and the plan nudge adds "Continue task now" steers on top. Keep `prewalk.enabled` off for the deepseek/free profiles. For a known-trivial default-profile session pass `omp --model opencode-go/deepseek-v4.1-flash`: the active model then equals the `@smol` target, whose pattern carries no level, and `prewalkWouldBeNoop` returns true on a matching model with an undefined target level — so prewalk disarms itself regardless of the active level. Do not reach for `--no-prewalk` there; it leaves `default` (astra) in place for the whole session.
- Work mirrors default's planning/implementation split with `anthropic/claude-fable-5-1` in the astra roles (`default`, `plan`, `slow` at `xhigh`, `advisor` at `high`) and the same deepseek `task`/`smol`/`tiny` roles, for machines that hold an Anthropic subscription instead of an OpenAI one. Every fable suffix used there was already in service in that file before the switch, so none of them relies on an untested rung of the model's effort ladder. Keep the two profiles structurally identical: a fix to one split (for example the suffix-free `smol`) applies to both.
- Link only config files, shared themes, and global agent instructions into each profile, never the entire profile or agent directory. Named profiles have separate stored authentication, sessions, and caches. Keep secrets out of tracked config. Existing configs are backed up as `config.yml.old` beside each destination rather than imported into Git. Adjacent `config.yml.lock`, `config-work.yml.lock`, `config-deepseek.yml.lock`, and `config-free.yml.lock` files are ignored.
- Renaming a named OMP profile means moving its whole directory, not just the tracked YAML: `~/.omp/profiles/<name>` holds `agent.db`, sessions, blobs, logs, and caches, while the config symlink points outside it into the checkout. `link-dotfiles.sh` renames `cheap` to `deepseek` under `~/.omp/profiles` and under any XDG data, state, or cache root that holds `omp/profiles/cheap`, preflighting every root first so a collision under a later root cannot leave an earlier one already moved. When both names exist it exits before moving anything and prints both paths, because either directory could hold the only copy of a login or a session history; a `cheap` entry that is not a real directory is refused for the same reason, since `mv` would relocate a symlink instead of the profile it points at.
- Symlink-write tests must use a disposable checkout so they cannot modify real tracked settings. For real OMP smoke checks, isolate `HOME` and XDG paths and clear profile/config override environment variables: named profiles ignore `PI_CODING_AGENT_DIR`, so that variable alone is not isolation. OMP config commands also open `agent.db`; let OMP create its database rather than supplying a text sentinel. Text runtime sentinels are safe only in linker-only tests that never invoke OMP. Compare resolved target identity in fixtures: macOS temporary paths may differ only by the `/private` prefix.
- Zed's `OMP`, `OMP Work`, `OMP DeepSeek`, and `OMP Free` entries pass `--profile default`, `--profile work`, `--profile deepseek`, and `--profile free` before `acp`, so inherited `OMP_PROFILE` values cannot change their selection. For an ACP smoke check, launch the exact configured command in an isolated `HOME` and send an `initialize` JSON-RPC request on raw stdin, terminated by a literal `\n`. A terminal Enter action may leave the raw input unterminated. Require a successful initialization response, not just a running process; this check needs no provider login or model request.
- Free pins every exposed role to `openrouter/stealth/union-alpha` with `retry.modelFallback` false. The model advertises no named reasoning-effort ladder, so role entries omit the `:effort` suffix and OMP uses the model's default effort. The OpenRouter stealth model list can change; if a launch reports an unknown model, refresh the provider cache and confirm the ID with `omp --profile free config get modelRoles --json`.
- To audit a profile's logins and role models: OMP has no `auth`/`login` subcommand (those words fall through to `launch`). Credentials sit in `~/.omp/profiles/<name>/agent/agent.db` table `auth_credentials` (`provider`, `credential_type`, `disabled_cause`, JSON `data` with `expires` in ms for OAuth); query expiry with `json_extract`, never print `data`. `omp --profile <name> models <provider>` lists only providers that profile can actually use, from a stored credential or a provider API key in the environment, so a missing provider means a missing login rather than a missing catalog entry. Listing no models at all is that same gap, not an empty catalog: the free profile reports `No models available. Set API keys in environment variables.` because it holds no stored credential and no `OPENROUTER_API_KEY`. Confirm each `modelRoles` id and suffix against that table, then prove auth end to end with `omp --profile <name> -p --no-session --no-tools --no-extensions --no-skills --no-rules --no-lsp --cwd /tmp --model <id[:effort]> "Reply with exactly the word: ok"`. Add `--mode json` to see the `thinking_level_changed` event (`configured: "auto"`, `resolved: <level>`) for `:auto` roles. A first `:auto` call against `opencode-go/deepseek-v4.1-flash` once stalled past 120 s while immediate reruns took 5–6 s at `resolved: low` (2026-09-18); rerun before treating a single stall as a config fault.
...
- Node is owned by the pinned nvm installer and tracks nvm's `lts/*` alias for the latest LTS release. The installer sets `nvm alias default 'lts/*'` so new shells follow LTS too. Rust is owned by rustup and tracks the stable channel. Keep the native package rows out of every platform catalog so one application has one toolchain owner.
- Sourcing `nvm.sh` normally runs automatic version selection. If the inherited `PATH` contains a Node binary whose reported version does not match its nvm directory, that selection returns 3 and aborts the installer before it can repair Node. Load nvm with `--no-use`; the following managed operation explicitly installs and selects the quoted `'lts/*'` alias.
...
- Application-level progress hides substantial work: one application can own several APT, Homebrew, npm, or Cargo entries, while direct toolchain installers can own several sequential phases. Build the operation registry before execution from every selected catalog action, declared installer phase, removal method, standalone setup step, and application verification. A shared package-manager batch may start and settle several registered operations together, but must not collapse them into one count. Direct installers that expose phases must execute them through `run_installer_operation` so the declared total and emitted lifecycle cannot drift.
...
- macOS names UID 0's primary group `wheel` and has no group named `root`. Linux usually names GID 0 `root`. Use numeric owner and group IDs with `install -o 0 -g 0` when creating a root-owned sudoers file on both platforms.
...
- Linux installation runs use standard process-scoped inhibitors rather than changing global desktop settings or simulating input. The logind inhibitor protects against OS idle sleep; GNOME's session inhibitor additionally prevents its independent idle lock. A successful `--list` does not prove acquisition is authorized: WSL2 can run systemd without associating the shell with an active logind seat, causing acquisition to request a missing `pkttyagent` and fail. Quietly probe each exact lock with non-interactive authorization, use only the inhibitors whose probes succeed, and never block setup when these optional facilities are unavailable. Keep the final Ghostty launch in the outer entry-point process so child-scoped inhibitors are released before the terminal starts; otherwise they remain held for the entire terminal session.
...
- A background `nohup` launched as the PTY-owning installer exits can lose a race with terminal teardown before the child starts. Use `setsid -f` for the Linux Ghostty launch when available. The portable fallback must fork a child that ignores `SIGHUP` and signal readiness only after installing that disposition; the parent waits for this handshake before it exits. Keep the PTY regression test polling the launch marker.
- PTY parents must continuously drain the master descriptor while waiting for a child. Linux happened to reap the visual-install test without doing so, while Darwin left the exited child unreaped until the master was read and produced a false ten-second launch hang.
...
## Yazi 26.9.1 package caches need a symlink migration

- Old package caches contain real Git symlinks. Yazi 26.9.1 expects regular files containing link targets, so reading an old symlink follows it and interprets Lua source as a path, failing with `File name too long`.
- Before package restoration, `lib/yazi.sh` restores only cached tracked symlink entries for that release with `core.symlinks=false`. Git's `checkout-index --force` alone keeps an unchanged symlink: unlink the entry first, then restore it from the index. Preserve the target file, untracked files, and the canonical package lockfile.
- The real-release integration check seeds the old cache and installs twice. Yazi 26.9.1 removed `--debug` and requires a terminal even for `--help`. The probe uses `--help`, which loads configuration first, in a bounded PTY session.
